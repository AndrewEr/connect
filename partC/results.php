  ...
  <?php
	...
	if(isset($_GET['submit']))
	{
		...
		if($errorstring == "")
		{
			...
			$result_query = "SELECT DISTINCT wine.wine_name AS Wine_Name,grape_variety.variety AS Grape_Variety, 
						wine.year AS Wine_Year, winery.winery_name AS Winery_Name,
						region.region_name AS Region_Name, items.price AS Price, 
						inventory.on_hand AS Quantity_Available, items.qty AS Quantity_Sold, 
						(items.qty * (items.price - inventory.cost))  AS Revenue
			...
			{
				$result_query .=" AND wine.wine_name LIKE '%$wineName%'";
			}
			if(isset($wineryName) && $wineryName != "All")
			{
				$result_query .=" AND winery.winery_name LIKE '%$wineryName%'";
			}
			if(isset($regionName) && $regionName != "All")
			{
				$result_query .=" AND region.region_name LIKE '%$regionName%'";
			}
			if(isset($variety) && $variety != "All")
			{
				$result_query .=" AND grape_variety.variety LIKE '%$variety%'";
			}
			...
			{
				$result_query .=" AND items.price >= '{$minCost}'";
			}
			if(isset($maxCost) && $maxCost != "All")
			{
				$result_query .=" AND items.price <= '{$maxCost}'";
			}	
    		...
    		{
    		 	echo "<br><br>Your search produced ".$rowsFound." results. Please go back and refine your search";
    		} 	
